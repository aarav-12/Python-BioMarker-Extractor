...
def _extract_value_and_unit(value_cell: str, unit_cell: str = "") -> tuple[float | None, str]:
    ...
    if value is None:
        nm = _NUM_SEARCH_RE.search(clean)
        if nm:
            value = float(nm.group(1))
        ...
        for page in pdf.pages:
            for table in (page.extract_tables() or []):
                ...
                for row in table:
                    ...
                    name_first_line = name_raw.strip().split("\n")[0]
                    if _SKIP_TABLE_RE.match(name_first_line):
                        continue

                    value, unit = _extract_value_and_unit(value_cell, unit_cell)
                    if value is None:
                        continue
